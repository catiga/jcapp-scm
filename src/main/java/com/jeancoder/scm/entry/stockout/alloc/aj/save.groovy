package com.jeancoder.scm.entry.stockout.alloc.aj
...
def pub_wh = JC.request.param('pub_wh');
def accept_wh = JC.request.param('accept_wh');
...
WareHouse pub_house = wh_service.get(pub_wh);
WareHouse acc_house = wh_service.get(accept_wh);
if(batch==null) {
	return SimpleAjax.notAvailable('batch_not_found');
}
if(pub_house==null||acc_house==null) {
	return SimpleAjax.notAvailable('warehouse_not_found');
}
if(pub_house.id==acc_house.id) {
	return SimpleAjax.notAvailable('warehouse_same_error');
}
...
def fail_ops = s_s.out_gs_form(batch, pub_house, acc_house, gs, ts, remark, user);
if(!fail_ops) {
	fail_ops = null;
}
