package com.jeancoder.scm.ready.gen;
...
public class GoodsNoGenerator {
	...
	static {
		try{
			...
			GoodsNoGenerator.ipp = ipnum.substring(ipnum.length()-2);
		}catch(Exception e){
		}
	}
	/**
	...
	public synchronized static String generateNo(){
		String newkey = new SimpleDateFormat("ddHHmmss").format(new Date());
		StringBuffer no = new StringBuffer(newkey);
		no.append(GoodsNoGenerator.ipp);
		
		if(newkey.equals(GoodsNoGenerator.key)){
			//同一秒计数累加
			no.append(String.format(numfmt, ++GoodsNoGenerator.cnt));
		}else{
			//新一秒计数归位
			GoodsNoGenerator.key = newkey;
			GoodsNoGenerator.cnt = 0;
			no.append(String.format(numfmt, ++GoodsNoGenerator.cnt));
		}
		return no.toString();
	}
