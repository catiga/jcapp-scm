...
apid = BigInteger.valueOf(Long.valueOf(apid));
AccountSession session = new AccountSession(ap_id:apid);
def tbn = JC.internal.param('tbn')?.toString()?.trim();
 ...
 * 190:店内自取
 */
def dss = JC.internal.param('dss')?.toString()?.trim();

println 'input_dss_value============' + dss + ', tbn=' + tbn;
if(!dss) {
	//默认为店内自取
	dss = '190';
}
...
def sid = JC.internal.param('sid')?.toString()?.trim();		//上传门店信息
...
order_addr.point = tbn;
def order_result = order_service.create_order_front(goods_list, store, dss, order_addr, pid)
