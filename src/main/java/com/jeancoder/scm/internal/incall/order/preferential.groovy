...
OrderInfo order = order_service.get(pid,oid);
if(order==null) {
...
for(x in items) {
	def tycode = x.tycode;
	if(tycode=='100') {
		GoodsInfo g = goods_service.get(x.goods_id,pid);
		if(!g) {
		...
		GoodsPack g = pack_goods_service.get_pack(x.goods_id,pid);
		if(!g) {
			continue;
		}
	} else if(tycode=='300') {
		GdMerge g = merge_goods_service.get(x.goods_id,pid);
		if(!g) {
			continue;
		}
	}
